"""
Petroleum Fluids Empirical Correlations and Utilites
...
def carr_gas_viscosity(P, T, gamma_g, y_N2=None, y_CO2=None, y_H2S=None):
    ...
    if y_N2 is None:
        ...
        mu_1N2 = (9.59e-3 + 8.48e-3 * np.log10(gamma_g)) * y_N2
    if y_CO2 is None:
        mu_1CO2 = 0
    else:
        mu_1CO2 = (6.24e-3 + 9.08e-3 * np.log10(gamma_g)) * y_CO2
    if y_H2S is None:
        mu_1H2S = 0
    else:
        mu_1H2S = (3.73e-3 + 8.49e-3 * np.log10(gamma_g)) * y_H2S
    
    #compute gas viscosity for hydrocarbon mixture at 1 atm.
    mu_1HC = 8.188e-3 - 6.15e-3 * np.log10(gamma_g) + \
    ...
    mu_g = (mu_1 / T_pr) * np.exp(mu_r)

    return mu_g

def hall_yarb_compressibility(P, T, gamma_g, y_CO2=None, y_H2S=None, 
                              tol=1e-6):
    """
    ...
    A = 0.06125 * t_r * np.exp(-1.2 * (1 - t_r) ** 2)
...
def spivey_oil_compressibility(P, P_b, T, API, gamma_gsp, R_sb):
    ...
    Z_1 = 3.011 - 2.6254 * np.log(API) + 0.497 * np.log(API) ** 2
    Z_2 = -0.0835 - 0.259 * np.log(gamma_gsp) + 0.382 * np.log(gamma_gsp) ** 2
    Z_3 = 3.51 - 0.0289 * np.log(P_b) - 0.0584 * np.log(P_b) ** 2
    Z_4 = 0.327 - 0.608 * np.log(P / P_b) + 0.0911 * np.log(P / P_b) ** 2
    Z_5 = -1.918 - 0.642 * np.log(R_sb) + 0.154 * np.log(R_sb) ** 2
    Z_6 = 2.52 - 2.73 * np.log(T) + 0.429 * np.log(T) ** 2
...
def mccain_oil_compressibility(P, P_b, T, API, R_sb):
    ...
    T += 459.67
    ln_c_o = -7.573 - 1.45 * np.log(P) - 0.383 * np.log(P_b) + 1.402 * \
        np.log(T) + 0.256 * np.log(API) + 0.449 * np.log(R_sb)

    return np.exp(ln_c_o)
...
def valko_mccain_p_b(T_res, API, gamma_g, R_sb):
    ...
    Z_1 = -5.48 - 0.0378 * np.log(R_sb) + 0.281 * np.log(R_sb) ** 2 -\
          0.0206 * np.log(R_sb) ** 3
...
def glaso_p_b(T_res, API, gamma_g, R_sb):
    ...
    log_P_b = 1.7669 + 1.7447 * np.log10(X) - 0.30218 * np.log10(X) ** 2
...
def vasquez_beggs_r_s(P, T, P_sep, T_sep, API, gamma_g):
    ...
    T += 459.67
    gamma_gn = normalized_gas_gravity(P_sep, T_sep, API, gamma_g)
    R_s = A * gamma_gn * P ** B * np.exp(C * (API / T))
...
def glaso_beta_o(T, API, gamma_g, R_s):
    ...
    beta_o = 1 + 10 ** (-6.58511 + 2.91329 * np.log10(X) - 0.27683 * \
                        np.log10(X) ** 2)
...
def undersaturated_beta_o(P, P_b, beta_ob, c_o):
    ...
    beta_o = beta_ob * np.exp(-c_o * (P - P_b))
...
def ng_egbogah_mu_od(T, API):
    ...
    log_10_mu_od = 1.8653 - 2.5086e-2 * API - 0.5644 * np.log10(T)
...
def bergman_sutton_mu_o(P, P_b, T, API, R_sb):
    ...
    alpha = 6.5698e-7 * np.log(mu_ob) ** 2 - 1.48211e-5 * np.log(mu_ob) + \
            2.27877e-4
    beta = 2.24623e-2 * np.log(mu_ob) + 0.873204
    mu_o = mu_ob * np.exp(alpha * (P - P_b) ** beta)
...
def vasquez_beggs_mu_o(P, P_b, T, API, R_sb):
    ...
    B = 2.6 * P ** 1.187 * np.exp(-11.513 - 8.98e-5 * P)
    mu_ob = beggs_robinson_mu_ob(T, API, R_sb)
    mu_o = mu_ob * (P / P_b) ** B
...
def glaso_mu_od(T, API):
    ...
    mu_od = 3.141e10 * T ** -3.444 * np.log10(API) ** A
